...
| --- | --- |
| **Minimização** | Importações devem conter somente o identificador do destinatário, o endereço do canal e as variáveis exigidas pelo template. Campos estranhos ao layout são rejeitados. |
...
| **Confidencialidade** | Endereços e variáveis de destinatários, credenciais de brokers e segredos de API são protegidos em repouso; respostas administrativas não devolvem o segredo integral. |
...
| E-mail, telefone ou identificador do destinatário | Entrega da comunicação | Conteúdo cifrado no banco e acesso por campanha escopada | 90 dias após conclusão terminal da campanha, configurável; depois é anonimizado |
| Variáveis de personalização | Renderização do template | JSON cifrado; não usado em dashboards | Mesmo prazo do destinatário; depois é removido |
...
| `RETENTION_RECIPIENT_PII_DAYS` | 90 | Destino e variáveis dos destinatários |
