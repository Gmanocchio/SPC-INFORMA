...
export const adminRouter = router({
  ...
  users: router({
    list: adminProcedure.input(z.object({ organizationId: z.number().int().positive().optional(), search: z.string().trim().max(120).optional(), includeManagedOrganizations: z.boolean().optional() })).query(({ ctx, input }) => listUsers(actor(ctx), input)),
