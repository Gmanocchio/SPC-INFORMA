...
import { KeyRound, Plus, Search, Shield, UserRoundCheck, UsersRound } from "lucide-react";
...
export default function Users() {
  ...
  const update = trpc.admin.users.update.useMutation({
    onSuccess: async () => { await utils.admin.users.list.invalidate(); toast.success("Situação do usuário atualizada."); },
    onError: error => toast.error(error.message),
  });
  ...
  return (
      <div className="space-y-6">
        ...
        <section className="command-panel p-4 md:p-6"><div className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h2 className="font-bold text-slate-950">Identidades autorizadas</h2><p className="text-sm text-slate-500">Perfis e situações são impostos também no servidor.</p></div><div className="relative w-full md:w-80"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><Input className="pl-9" value={search} onChange={event => setSearch(event.target.value)} placeholder="Buscar por nome, CPF ou e-mail" /></div></div>
          {users.isLoading ? <div className="space-y-3"><Skeleton className="h-12 w-full" /><Skeleton className="h-12 w-full" /></div> : users.isError ? <QueryErrorState message={users.error.message} onRetry={() => void users.refetch()} /> : users.data?.length ? <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Usuário</TableHead><TableHead>Perfil</TableHead><TableHead>Segurança</TableHead><TableHead>Situação</TableHead><TableHead>Último acesso</TableHead><TableHead className="text-right">Ação</TableHead></TableRow></TableHeader><TableBody>{users.data.map(user => <TableRow key={user.id}><TableCell><div className="font-semibold text-slate-900">{user.name}</div><div className="text-xs text-slate-500">{user.email}</div></TableCell><TableCell><span className="inline-flex items-center gap-1.5 text-sm"><Shield className="size-3.5 text-[#0066cc]" />{roleLabels[user.role]}</span></TableCell><TableCell>{user.mustChangePassword ? <Badge className="bg-amber-50 text-amber-700"><KeyRound className="mr-1 size-3" />Troca pendente</Badge> : <Badge className="bg-emerald-50 text-emerald-700"><UserRoundCheck className="mr-1 size-3" />Verificado</Badge>}</TableCell><TableCell><Badge variant="secondary">{statusLabels[user.status]}</Badge></TableCell><TableCell className="text-sm text-slate-500">{user.lastSignedIn ? new Date(user.lastSignedIn).toLocaleString("pt-BR") : "Nunca"}</TableCell><TableCell className="text-right"><Select value={user.status} disabled={user.id === identity?.user.id} onValueChange={status => update.mutate({ id: user.id, data: { status: status as "INVITED" | "ACTIVE" | "INACTIVE" | "LOCKED" } })}><SelectTrigger className="ml-auto w-36"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ACTIVE">Ativar</SelectItem><SelectItem value="INACTIVE">Inativar</SelectItem><SelectItem value="LOCKED">Bloquear</SelectItem><SelectItem value="INVITED">Primeiro acesso</SelectItem></SelectContent></Select></TableCell></TableRow>)}</TableBody></Table></div> : <div className="flex min-h-60 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 p-8 text-center"><UsersRound className="size-10 text-[#0066cc]" /><h3 className="mt-4 font-bold text-slate-900">Nenhum usuário encontrado</h3><p className="mt-1 max-w-md text-sm text-slate-500">Cadastre o primeiro usuário deste escopo ou ajuste a busca.</p></div>}
        </section>
      </div>
  );
}
