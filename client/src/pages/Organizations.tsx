...
import { Building2, CircleDollarSign, ImageUp, MapPin, Plus, Search, ShieldCheck } from "lucide-react";
...
export default function Organizations() {
  ...
  const [form, setForm] = useState({ type: "CREDITOR" as "SPC_BRASIL" | "CDL" | "DISTRIBUTOR" | "CREDITOR", legalName: "", tradeName: "", cnpj: "", responsibleName: "", responsibleEmail: "", responsiblePhone: "", postalCode: "", street: "", streetNumber: "", addressExtra: "", district: "", city: "", state: "", billingModel: "PREPAID" as "PREPAID" | "POSTPAID", balance: "0", creditLimit: "0" });
        ...
        <section className="command-panel overflow-hidden p-6 md:p-8">
          <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              <DialogTrigger asChild><Button className="h-11 bg-[#0066cc] px-5 text-white hover:bg-[#004a99]"><Plus className="size-4" /> Nova empresa</Button></DialogTrigger>
              <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
                <DialogHeader><DialogTitle>Cadastrar empresa</DialogTitle><DialogDescription>Informe dados cadastrais, responsável e regime financeiro. Campos obrigatórios são validados no servidor.</DialogDescription></DialogHeader>
                <form className="grid gap-4 pt-2 sm:grid-cols-2" onSubmit={submit}>
                  {isSpcAdmin && <Field label="Tipo"><Select value={form.type} onValueChange={value => setForm(current => ({ ...current, type: value as typeof form.type }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="CDL">CDL</SelectItem><SelectItem value="DISTRIBUTOR">Distribuidora</SelectItem><SelectItem value="CREDITOR">Credor</SelectItem><SelectItem value="SPC_BRASIL">SPC Brasil</SelectItem></SelectContent></Select></Field>}
                  <Field label="CNPJ"><Input required value={form.cnpj} onChange={event => setForm({ ...form, cnpj: event.target.value })} placeholder="00.000.000/0000-00" /></Field>
                  ...
                  <div className="flex justify-end gap-3 border-t pt-4 sm:col-span-2"><Button type="button" variant="outline" onClick={() => setOpen(false)}>Cancelar</Button><Button disabled={create.isPending} className="bg-[#0066cc] text-white">{create.isPending ? "Salvando…" : "Cadastrar empresa"}</Button></div>
                </form>
              </DialogContent>
            </Dialog>
          </div>
