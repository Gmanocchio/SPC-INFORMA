            ...
            <Dialog open={open} onOpenChange={next => { setOpen(next); if (!next) setEditingId(null); }}>
              ...
              <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
                ...
                <form className="grid gap-4 pt-2 sm:grid-cols-2" onSubmit={submit}>
                  ...
                  {form.type === "CREDITOR" && isSpcAdmin && organizations.data && <Field label="Vinculado a"><Select value={form.linkedToOrganizationId || ""} onValueChange={value => setForm(current => ({ ...current, linkedToOrganizationId: value }))}><SelectTrigger><SelectValue placeholder="Selecione uma CDL, Distribuidora ou deixe em branco para SPC" /></SelectTrigger><SelectContent><SelectItem value="">SPC Brasil</SelectItem>{organizations.data.filter(o => o.type === "CDL" || o.type === "DISTRIBUTOR").map(o => <SelectItem key={o.id} value={String(o.id)}>{o.tradeName} ({typeLabels[o.type]})</SelectItem>)}</SelectContent></Select></Field>}
